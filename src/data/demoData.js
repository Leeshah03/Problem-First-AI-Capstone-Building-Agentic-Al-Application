// Demo/seed data used when the backend API is unavailable (e.g. static Vercel deploy)
export const DEMO_THEMES = [
  {
    ...
    pm: "Priya Nair",
    revenueImpact: 92, strategicFit: 95, competitiveDiff: 75, signalFrequency: 88,
...
// Demo connectors data
const now = new Date();
const hours = (h) => new Date(now.getTime() - h * 3600000).toISOString();

export const DEMO_CONNECTORS = [
  {
    id: "conn-gong", name: "Gong", type: "gong", enabled: true,
    syncFrequency: "1h", lastSyncAt: hours(1),
    recentJobs: [
      { id: "j1", status: "completed", signals_new: 6, signals_pulled: 42, completed_at: hours(1) },
      { id: "j2", status: "completed", signals_new: 3, signals_pulled: 36, completed_at: hours(2) },
      { id: "j3", status: "completed", signals_new: 5, signals_pulled: 33, completed_at: hours(3) },
    ],
  },
  {
    id: "conn-canny", name: "Canny", type: "canny", enabled: true,
    syncFrequency: "24h", lastSyncAt: hours(8),
    recentJobs: [
      { id: "j4", status: "completed", signals_new: 14, signals_pulled: 4329, completed_at: hours(8) },
      { id: "j5", status: "completed", signals_new: 22, signals_pulled: 4315, completed_at: hours(32) },
    ],
  },
  {
    id: "conn-pendo", name: "Pendo", type: "pendo", enabled: true,
    syncFrequency: "6h", lastSyncAt: hours(3),
    recentJobs: [
      { id: "j6", status: "completed", signals_new: 4, signals_pulled: 18, completed_at: hours(3) },
      { id: "j7", status: "completed", signals_new: 5, signals_pulled: 14, completed_at: hours(9) },
      { id: "j8", status: "failed", signals_new: 0, signals_pulled: 0, completed_at: hours(15) },
    ],
  },
  {
    id: "conn-salesforce", name: "Salesforce", type: "salesforce", enabled: true,
    syncFrequency: "6h", lastSyncAt: hours(2),
    recentJobs: [
      { id: "j9", status: "completed", signals_new: 3, signals_pulled: 12, completed_at: hours(2) },
      { id: "j10", status: "completed", signals_new: 5, signals_pulled: 9, completed_at: hours(8) },
    ],
  },
  {
    id: "conn-jira", name: "Jira", type: "jira", enabled: true,
    syncFrequency: "1h", lastSyncAt: hours(0.5),
    recentJobs: [
      { id: "j11", status: "completed", signals_new: 4, signals_pulled: 28, completed_at: hours(0.5) },
      { id: "j12", status: "completed", signals_new: 3, signals_pulled: 24, completed_at: hours(1.5) },
      { id: "j13", status: "completed", signals_new: 6, signals_pulled: 21, completed_at: hours(2.5) },
    ],
  },
];
...
export const DEMO_ACTIVITY = [
  { type: "sync", connectorName: "Jira", connectorType: "jira", status: "completed", signalsNew: 4, signalsPulled: 28, totalSignals: 28, classifiedSignals: 0, timestamp: hours(0.5) },
  { type: "sync", connectorName: "Gong", connectorType: "gong", status: "completed", signalsNew: 6, signalsPulled: 42, totalSignals: 42, classifiedSignals: 0, timestamp: hours(1) },
  { type: "pipeline", connectorName: null, connectorType: null, status: "completed", signalsNew: 0, signalsPulled: 0, totalSignals: 10, classifiedSignals: 10, timestamp: hours(1.2) },
  { type: "sync", connectorName: "Salesforce", connectorType: "salesforce", status: "completed", signalsNew: 3, signalsPulled: 12, totalSignals: 12, classifiedSignals: 0, timestamp: hours(2) },
  { type: "sync", connectorName: "Pendo", connectorType: "pendo", status: "completed", signalsNew: 4, signalsPulled: 18, totalSignals: 18, classifiedSignals: 0, timestamp: hours(3) },
  { type: "sync", connectorName: "Canny", connectorType: "canny", status: "completed", signalsNew: 14, signalsPulled: 4329, totalSignals: 4329, classifiedSignals: 0, timestamp: hours(8) },
  { type: "pipeline", connectorName: null, connectorType: null, status: "completed", signalsNew: 0, signalsPulled: 0, totalSignals: 22, classifiedSignals: 22, timestamp: hours(8.5) },
  { type: "sync", connectorName: "Pendo", connectorType: "pendo", status: "failed", signalsNew: 0, signalsPulled: 0, totalSignals: 0, classifiedSignals: 0, timestamp: hours(15) },
];

// Demo review queue items
export const DEMO_REVIEW_ITEMS = [
  {
    id: "rv-1", reviewType: "low_confidence", status: "pending", aiConfidence: 0.42,
    signal: { source: "gong", content: "We need better noise reduction controls. The current one-click approach doesn't work for our professional audio.", speaker: "Eli Washington, Audio Engineer", account: "SoundByte Studios", biasTag: "CLEAN" },
    themes: [{ themeId: "t8", themeName: "Pro Audio Controls" }],
  },
  {
    id: "rv-2", reviewType: "bias_conflict", status: "pending", aiConfidence: 0.67, aiSuggestedBias: "INTERNAL",
    signal: { source: "gong", content: "Have you considered using our new AI clip finder? It could really help with your workflow for finding highlights.", speaker: "Sam Korova, Head of Production", account: "BrightWave Media", biasTag: "CLEAN" },
    themes: [{ themeId: "t4", themeName: "AI Clip Finding & Cross-Project Intelligence" }],
  },
  {
    id: "rv-3", reviewType: "duplicate_candidate", status: "pending", aiConfidence: 0.73,
    signal: { source: "canny", title: "Better transcription for medical terms", content: "Better transcription for medical terms — accuracy drops significantly with specialized vocabulary", biasTag: null },
    themes: [{ themeId: "t1", themeName: "Transcription Accuracy & Domain Vocabulary" }],
  },
  {
    id: "rv-4", reviewType: "low_confidence", status: "pending", aiConfidence: 0.38,
    signal: { source: "salesforce", content: "Expansion deal blocked by lack of RBAC. Cannot onboard new team members without role controls.", speaker: "Marcus Johnson", account: "Apex Digital", biasTag: null },
    themes: [{ themeId: "t3", themeName: "Collaboration & Review Workflows" }],
  },
  {
    id: "rv-5", reviewType: "new_theme", status: "pending", aiConfidence: 0.55,
    signal: { source: "pendo", content: "NPS 4/10 — Detractor: Export times are getting worse with each update. 45 min for a 30 min video is unacceptable.", speaker: null, account: "CloudNine Media", biasTag: null },
    themes: [{ themeId: "t7", themeName: "Timeline Performance & Track Management" }],
  },
  {
    id: "rv-6", reviewType: "bias_conflict", status: "pending", aiConfidence: 0.61, aiSuggestedBias: "WHALE",
    signal: { source: "gong", content: "We'd switch from Riverside tomorrow if you supported real-time AI translation during live recordings. We'd put all 2,000 seats on Descript.", speaker: "Rachel Simmons, Content Lead", account: "TechVault Inc.", biasTag: "COMPETITIVE" },
    themes: [{ themeId: "t13", themeName: "AI Voice Quality & Third-Party Voice Integration" }],
  },
  {
    id: "rv-7", reviewType: "low_confidence", status: "pending", aiConfidence: 0.44,
    signal: { source: "jira", content: "[High] Timeline freezes when adding 15+ audio tracks. Reported by alex.kim. Component: Timeline Editor. Affecting 32 users.", speaker: null, account: null, biasTag: null },
    themes: [{ themeId: "t7", themeName: "Timeline Performance & Track Management" }],
  },
  {
    id: "rv-8", reviewType: "duplicate_candidate", status: "pending", aiConfidence: 0.81,
    signal: { source: "canny", title: "Real-time collaboration with presence indicators", content: "Real-time collaboration with presence indicators — need to see who is editing what section", biasTag: null },
    themes: [{ themeId: "t3", themeName: "Collaboration & Review Workflows" }],
  },
  {
    id: "rv-9", reviewType: "low_confidence", status: "pending", aiConfidence: 0.35,
    signal: { source: "pendo", content: "Feature \"Green Screen\" saw a 38% drop in weekly active users compared to last month. 27 accounts affected.", speaker: null, account: "Prism Podcasts", biasTag: null },
    themes: [],
  },
  {
    id: "rv-10", reviewType: "new_theme", status: "pending", aiConfidence: 0.52,
    signal: { source: "salesforce", content: "Customer requesting custom API integration for automated publishing workflow. ARR at stake: $90,000.", speaker: "Emily Rodriguez", account: "Apex Digital", biasTag: null },
    themes: [{ themeId: "t12", themeName: "API & Headless Export Pipeline" }],
  },
  {
    id: "rv-11", reviewType: "bias_conflict", status: "pending", aiConfidence: 0.58, aiSuggestedBias: "SOLUTION",
    signal: { source: "gong", content: "What if you built a keyframe editor with easing curves? That would solve our animation needs completely.", speaker: "Aisha Brooks, Director of Content", account: "Streamline Studios", biasTag: "CLEAN" },
    themes: [{ themeId: "t10", themeName: "Motion, Animation & Visual Polish" }],
  },
  {
    id: "rv-12", reviewType: "low_confidence", status: "pending", aiConfidence: 0.41,
    signal: { source: "jira", content: "ESCALATION: MediaStack Inc. — Export/Render. Production team blocked: export queue stuck for 4+ hours. Deadline tomorrow.", speaker: null, account: "MediaStack Inc.", biasTag: null },
    themes: [{ themeId: "t5", themeName: "Accessibility & Compliance Automation" }, { themeId: "t7", themeName: "Timeline Performance & Track Management" }],
  },
];
